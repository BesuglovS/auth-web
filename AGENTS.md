...
4. **`api/progress.php`** поддерживает server-to-server переопределение `user_id` (запросы без `Origin`
   ...
   через override.
5. **CSRF**: все form-POST требуют сменный токен. **Rate-limit входа ЕСТЬ**: `Auth::login()`
   ограничивает неудачные попытки (10 за 5 минут на логин и на IP, таблица `rate_limits`,
   см. `LOGIN_RATE_*` в `includes/Auth.php`) — не удаляйте и не ослабляйте его.
